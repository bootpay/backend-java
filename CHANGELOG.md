...
주면 그 요청의 발송 성공·실패·문자 대체발송·예약취소 웹훅이 **그 주소로만** 가고 프로젝트 웹훅 설정은 쓰이지 않는다.
미지정이면 바디에서 빠져 지금까지처럼 프로젝트 웹훅 설정을 따른다.

`https` 만 허용하고 2,000자를 넘으면 3028 로 거부된다 (벌크는 요청 전체가 거부된다). 서명은 프로젝트 시크릿으로
하므로 시크릿만 필요하면 `alimtalkWebhook.rotateSecret()` 으로 설정 없이 발급받을 수 있다.

> ⚠️ 같은 `refId` 로 이미 접수·성공한 건을 다시 요청하면 기존 접수가 그대로 돌아와 새 주소는 무시된다.

#### 알림톡 API 를 메시지 호스트로 전환 (ruby SDK parity)

`alimtalk/*` 경로는 이제 커머스 API(`api.bootapi.com/v1`)가 아니라 메시지 API 가 받는다 (경로에 `/v1` 없음).
경로·파라미터·응답은 그대로이고 호스트만 다르다. 옛 주소(`/v1/alimtalk/*`)는 410 으로 응답한다.
